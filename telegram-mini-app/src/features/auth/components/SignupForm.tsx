...
const vendorSchema = z
  .object({
    ...
    contactNumber: z
      .string()
      .regex(/^(09|07)\d{8}$/, "Must be a valid Ethiopian phone number"),
    password: z
      .string()
      .min(8, "Password Is Too Short!")
      .regex(/[A-Z]/, "Must Contain Uppercase!")
...
export default function SignupForm() {
  ...
  const {
    ...
    resolver: zodResolver(
      signupMode === "student" ? studentSchema : vendorSchema,
    ),
    mode: "onChange",
  });
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 font-sans flex flex-col items-center pt-16">
      ...
      <div className="w-full max-w-[340px] px-4 flex flex-col items-center pb-20">
        ...
        <div className="w-full bg-gray-50 dark:bg-gray-700 border border-gray-100 dark:border-gray-600 p-1 rounded-[14px] flex mb-6">
          ...
          >
            <User size={16} strokeWidth={signupMode === "student" ? 2.5 : 2} />{" "}
            Student
          </button>
          <button
            type="button"
          ...
          >
            <Store size={16} strokeWidth={signupMode === "vendor" ? 2.5 : 2} />{" "}
            Vendor
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-4">
          ...
          <div className="space-y-1">
            ...
            <input
              {...register(
                signupMode === "student" ? "fullName" : "vendorName",
              )}
              placeholder={
                signupMode === "student" ? "John Doe" : "Restaurant Name"
              }
              className={cn(
                "w-full h-13 border rounded-[10px] px-5 text-[15px] font-medium text-gray-900 dark:text-gray-300 focus:border-[#F26A1C] focus:outline-none transition-all",
                studentErrors.fullName || vendorErrors.vendorName
                  ? "border-red-500"
                  : "border-gray-200 dark:border-gray-600",
              )}
            />
            {signupMode === "student" && studentErrors.fullName && (
              <p className="text-xs text-red-500 font-semibold ml-1">
                {studentErrors.fullName.message}
              </p>
            )}
            {signupMode === "vendor" && vendorErrors.vendorName && (
              <p className="text-xs text-red-500 font-semibold ml-1">
                {vendorErrors.vendorName.message}
              </p>
            )}
          </div>
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder={
                signupMode === "student"
                  ? "name.surname@astu.edu.et"
                  : "contact@business.com"
              }
              className={cn(
                "w-full h-13 border rounded-[10px] px-5 text-[15px] font-medium text-gray-900 dark:text-gray-300 focus:border-[#F26A1C] focus:outline-none transition-all",
                studentErrors.astuEmail || vendorErrors.email
                  ? "border-red-500"
                  : "border-gray-200 dark:border-gray-600",
              )}
            />
            {signupMode === "student" && studentErrors.astuEmail && (
              <p className="text-xs text-red-500 font-semibold ml-1">
                {studentErrors.astuEmail.message}
              </p>
            )}
            {signupMode === "vendor" && vendorErrors.email && (
              <p className="text-xs text-red-500 font-semibold ml-1">
          ...
          {signupMode === "vendor" && (
            <div className="space-y-1">
              ...
              {vendorErrors.contactNumber && (
                <p className="text-xs text-red-500 font-semibold ml-1">
                  {vendorErrors.contactNumber.message}
                </p>
              )}
            </div>
          )}
