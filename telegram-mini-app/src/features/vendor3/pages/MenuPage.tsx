...
export default function MenuPage() {
  ...
  const { items, toggleItemStock, fetchMenu, isLoading } = useVendorMenuStore();
  const [activeTab, setActiveTab] = useState<"available" | "unavailable">("available");

  // Fetch the menu data from the backend when the page loads
  useEffect(() => {
    fetchMenu();
  }, [fetchMenu]);

  // CRITICAL FIX: Changed `item.inStock` to `item.isAvailable` to match the backend DTO
          ...
          <div key={item.id} className="relative bg-white rounded-[32px] p-4 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-50">
            ...
            <div className="flex items-center gap-4 border-b border-gray-50 pb-4">
              ...
              <div className="flex-1 flex justify-between items-center">
                <h3 className={`font-black text-gray-900 ${item.isAvailable ? '' : 'text-gray-400'}`}>{item.name}</h3>
            ...
            <div className="flex gap-2 mt-4 px-1">
              <button 
                ...
                className="flex items-center justify-center gap-1.5 bg-orange-500 text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider shadow-lg shadow-orange-100 active:scale-95 transition-all flex-1"
              >
                <Pencil className="h-3 w-3" /> Edit
              </button>
              <button 
                onClick={() => toggleItemStock(item.id, !item.isAvailable)}
                className={`flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider border-2 active:scale-95 transition-all flex-1
                  ${item.isAvailable 
                    ? 'bg-white border-orange-500 text-orange-500 hover:bg-orange-50' 
                    : 'bg-green-500 border-green-500 text-white'}`}
              >
                {item.isAvailable ? <><EyeOff className="h-3 w-3" /> Hide</> : <><Eye className="h-3 w-3" /> Show</>}
              </button>
            </div>
