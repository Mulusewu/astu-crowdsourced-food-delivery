...
import {
  useVendorOrderStore,
  type KitchenOrder,
} from "@/store/vendor/vendorOrderStore";
...
export default function VendorDashboard() {
  ...
  const { currentRestaurant, fetchRestaurantDetails, toggleRestaurantStatus } =
    useRestaurantStore();
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 px-6 text-center">
        ...
        <p className="text-gray-500 font-medium text-[15px] leading-relaxed max-w-sm">
          Your vendor account has been created successfully. An administrator
          must verify your business license and assign you to a restaurant
  ...
  const pendingAcceptance = kitchenQueue.filter(
    (o) => o.status === "AWAITING_VENDOR",
  );
  const waitingOnLogistics = kitchenQueue.filter((o) =>
    ["AWAITING_ACCEPT", "ASSIGNED", "AWAITING_PAYMENT"].includes(o.status),
  );
  const readyToCook = kitchenQueue.filter(
    (o) => o.status === "PAYMENT_RECEIVED",
  );
  const currentlyCooking = kitchenQueue.filter(
    (o) => o.status === "VENDOR_BEING_PREPARED",
  );

  const isOpen =
    currentRestaurant?.manualIsOpen ?? currentRestaurant?.isOpen ?? false;
  ...
  const PremiumOrderCard = ({
    ...
    const timeElapsedMins = Math.round(
    ...
    return (
      ...
      >
        <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-28 h-28 rounded-full border-[6px] border-white shadow-xl overflow-hidden z-10 transition-transform group-hover:scale-110 duration-500">
          <img
            src={firstImage}
            alt={`Order ${order.shortId}`}
            className="w-full h-full object-cover"
          />
        </div>

        <div
          className={`absolute right-4 top-4 px-2 py-0.5 rounded-full border z-20 ${badgeColor}`}
        >
        ...
        <div className="flex flex-col items-center text-center gap-0.5 mt-2">
          ...
          <div className="flex flex-col items-center leading-tight">
            ...
            <div className="mt-2 bg-gray-50 px-3 py-1 rounded-full border border-gray-100 flex items-center gap-2">
              ...
              <span className="text-primary/90 text-[11px] font-black">
        ...
        <div className="mt-5 w-full flex gap-2">
          ...
          >
            {isProcessing ? (
              ...
              disabledText || primaryActionText
            )}
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6 p-6 pb-28 max-w-lg mx-auto w-full bg-white min-h-screen animate-in fade-in slide-in-from-bottom-2 duration-500">
        ...
        <div className="flex flex-col">
          <h1 className="text-2xl font-bold leading-tight">
            ...
            <span className="text-black">
              {currentRestaurant?.name.split(" ")[0] ||
                firstName(user?.fullName || "Vendor")}
            </span>
          </h1>
        </div>
        <div className="group relative">
          <div className="h-12 w-12 rounded-full bg-primary flex items-center justify-center text-white text-xl font-bold shadow-lg cursor-pointer hover:bg-primary/90 transition-all hover:scale-105 active:scale-95 overflow-hidden">
            {user?.avatarUrl || currentRestaurant?.imageUrl ? (
              <img
                src={user?.avatarUrl || currentRestaurant?.imageUrl}
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-2 h-2 rounded-full ${isOpen ? "bg-green-500 animate-pulse" : "bg-gray-400"}`}
              ></div>
              <span
                className={`text-lg font-black ${isOpen ? "text-green-600" : "text-gray-500"}`}
              >
                {isOpen ? "OPEN" : "CLOSED"}
              </span>
            </div>
          ...
          <button
            ...
            className={`relative w-14 h-7 rounded-full transition-colors duration-300 ${isOpen ? "bg-primary" : "bg-gray-300"}`}
          >
            <div
              className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-transform duration-300 shadow-sm ${isOpen ? "translate-x-8" : "translate-x-1"}`}
            ></div>
          </button>
        </div>
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-16 mt-8">
            {waitingOnLogistics.map((order) => (
              <PremiumOrderCard
                ...
                order={order}
                badgeText={
                  order.status === "AWAITING_PAYMENT" ? "UNPAID" : "NO DRIVER"
                }
      ...
      <div className="flex flex-col gap-4 mt-8">
        ...
        {readyToCook.length === 0 ? (
          <p className="text-sm text-gray-400 italic">
            No paid orders waiting to be cooked.
          </p>
        ) : (
          <div className="grid grid-cols-2 gap-x-4 gap-y-16 mt-8">
        ...
        {currentlyCooking.length === 0 ? (
          ...
          <div className="space-y-3">
            {currentlyCooking.map((order) => (
              <div
                key={order.id}
                className="bg-orange-50 border border-orange-100 rounded-2xl p-4 flex justify-between items-center group active:scale-[0.98] transition-transform cursor-pointer"
              >
      ...
      >
        <SheetContent
          side="bottom"
          className="rounded-t-[32px] pb-8 px-6 pt-6 bg-gray-50 flex flex-col gap-4 max-h-[85vh] sm:max-w-lg sm:mx-auto sm:border-x sm:border-gray-200"
        >
          {previewOrder && (
            <>
              ...
              <div className="flex-1 overflow-y-auto flex flex-col gap-3 py-2 -mx-2 px-2">
                ...
                {previewOrder.items.map((item, idx) => (
                  <div
                    key={idx}
                    className="bg-white rounded-2xl p-3 shadow-[0_2px_10px_rgb(0,0,0,0.02)] border border-gray-100 flex items-center gap-4"
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-1 flex items-center gap-2">
                        <span className="text-xs font-black text-[#F26A1C] bg-orange-50 px-2 py-0.5 rounded-md">
                          x{item.quantity}
                        </span>
                      </div>
                    </div>
