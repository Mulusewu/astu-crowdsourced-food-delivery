...
export default function AvailableDeliveryDetailsPage() {
  ...
  useEffect(() => {
    ...
    return () => clearState(); // Cleanup on unmount
  }, [orderId]);

  const handleDecline = () => {
    try {
      declineOrder(navigate);
      toast.success("Order removed from your feed.");
    } catch (error) {
  ...
  const handleAccept = async () => {
    try {
      // The store handles the backend call, state refresh, and navigation
      await acceptOrder(navigate);
      toast.success("Order Accepted! Waiting for customer payment.");
    } catch (error: any) {
      toast.error(error.message || "Someone else may have accepted this order.");
        ...
        <div className="relative flex items-center justify-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-black text-gray-900 dark:text-white tracking-wide">
              {order.restaurantName || "Helen Cafe"}
            </h1>
          </div>
        </div>
      </header>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-[32px] p-6 shadow-[0_4px_25px_rgba(0,0,0,0.04)]">
          ...
          <div className="flex gap-4">
            <button
              ...
              disabled={isAccepting || order.status !== "AWAITING_ACCEPT"}
