...
export default function DeliveryDashboard() {
  ...
  const { orders, activeOrders, fetchAvailableOrders, fetchActiveOrders,connectDispatchSocket,disconnectDispatchSocket } =
    useOrderStore();
  ...
  const handleToggleActive = async () => {
    try {
      ...
      if (!delivererProfile?.isAvailable) {
        ...
        disconnectDispatchSocket(); // Disconnect to stop receiving updates when offline
      }
    } catch (error: any) {
