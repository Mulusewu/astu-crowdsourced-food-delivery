...
/**
 * FoodDetailsPage
 * Features: TMA BackButton, Haptic Feedback, and Mobile-Optimized Layout.
 */
export default function FoodDetailsPage() {
  ...
  if (error || !currentFood) return <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4 text-center"><AlertCircle size={32} className="text-destructive mb-4" /><h2 className="text-xl font-semibold mb-2">Something went wrong</h2><Button onClick={handleBack} variant="outline">Go Back</Button></div>;
    ...
    <div className="min-h-screen bg-background pb-28 relative">
      <div className="px-5 pt-10">
        <div className="relative h-[300px] w-full rounded-[48px] overflow-hidden shadow-xl border-4 border-white dark:border-gray-900 group">
          <img src={currentFood.image.replace("w=200", "w=800")} alt={currentFood.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
          ...
          <div className="absolute bottom-6 left-6 z-10 flex flex-col gap-2">
            {currentFood.isPopular && <Badge className="bg-primary text-white border-0 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest"><Flame size={12} className="mr-1" /> Popular</Badge>}
      ...
      <div className="-mt-6 relative z-20 px-5">
        <div className="mb-4">
          <h2 className="text-2xl font-bold text-foreground tracking-tight mb-1">{currentFood.name}</h2>
          <p className="text-sm text-muted-foreground font-medium">{currentFood.restaurantName}</p>
        </div>
        <div className="bg-gray-50 dark:bg-gray-900/50 rounded-[32px] p-5 flex justify-between items-center mb-8 border border-gray-100 dark:border-gray-800">
          <div className="flex flex-col items-center flex-1"><span className="text-lg font-bold text-primary">ETB {finalPrice}</span><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Price</span></div>
          <div className="w-[1px] h-10 bg-border" />
          <div className="flex flex-col items-center flex-1"><div className="flex items-center gap-1"><Star size={14} className="fill-primary text-primary" /><span className="text-lg font-bold">{currentFood.rating}</span></div><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{currentFood.totalReviews} Reviews</span></div>
          <div className="w-[1px] h-10 bg-border" />
          <div className="flex flex-col items-center flex-1"><div className="flex items-center gap-1"><Clock size={14} className="text-primary" /><span className="text-lg font-bold">{currentFood.preparationTime}</span></div><span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">Minutes</span></div>
        </div>
        <div className="mb-6"><h3 className="text-sm font-semibold mb-2">Description</h3><p className="text-sm text-muted-foreground leading-relaxed">{currentFood.description}</p></div>
        <div className="space-y-2 mb-6"><label className="text-sm font-medium">Special Instructions (Optional)</label><textarea value={specialInstructions} onChange={(e) => setSpecialInstructions(e.target.value)} className="w-full h-28 bg-background border border-border rounded-xl p-3 text-sm focus:ring-2 focus:ring-primary/50 transition-all resize-none" placeholder="e.g., No onions, extra spicy..." /></div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-card border-t border-border px-5 py-4 pb-6 flex items-center justify-between gap-4 z-50">
        <div className="bg-muted rounded-xl flex items-center p-1">
          <button onClick={handleDecrease} disabled={quantity <= 1} className="w-9 h-9 flex items-center justify-center rounded-lg disabled:opacity-50 active:scale-95 transition-all"><Minus size={16} /></button>
          <span className="w-10 text-center font-semibold">{quantity}</span>
          <button onClick={handleIncrease} disabled={quantity >= 10} className="w-9 h-9 flex items-center justify-center rounded-lg text-primary active:scale-95 transition-all"><Plus size={16} /></button>
        </div>
        <button onClick={handleAddToCart} className="flex-1 bg-primary text-primary-foreground rounded-xl px-6 py-3 font-semibold text-sm shadow-lg active:scale-95 transition-all">Add to Cart • ETB {finalPrice * quantity}</button>
      </div>
      ...
      {showSwitchModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-6 animate-in fade-in duration-300">
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-[32px] p-6 w-full max-w-sm shadow-2xl animate-in zoom-in-95">
            ...
            <p className="text-[13px] font-medium text-gray-500 text-center mb-8 px-2">Your cart already contains items from <span className="font-bold text-gray-900 dark:text-white">{cartRestaurant?.name}</span>. Adding this will clear your current cart.</p>
