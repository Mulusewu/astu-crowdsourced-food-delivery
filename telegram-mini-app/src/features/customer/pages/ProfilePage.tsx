...
import { 
  ChevronRight, Lock, CreditCard, ArrowLeft, Camera, Search, MapPin, Check, Plus
} from "lucide-react";
...
export default function CustomerProfileFlow() {
  ...
  const [activeView, setActiveView] = useState<
    "menu" | "edit" | "address" | "password" | "payment" | 
    "add-payment-list" | "add-telebirr-form" | "add-telebirr-success"
  >("menu");
  ...
  const Header = ({ title, showBack, onBackClick }: { title: string, showBack?: boolean, onBackClick?: () => void }) => (
    <div className="relative flex items-center justify-center pt-6 pb-4 mb-2">
      ...
      <h1 className="font-bold text-lg text-gray-900 dark:text-white tracking-wide">{title}</h1>
    </div>
  );
      ...
      <div className="min-h-screen bg-[#FDFDFD] dark:bg-gray-950 px-5 pb-28 font-sans flex flex-col">
        ...
        <div className="flex flex-col items-center mt-4 mb-8">
          <Avatar className="w-24 h-24 border-[3px] border-white dark:border-gray-900 shadow-md mb-3">
            <AvatarImage src={user?.avatar || "https://i.pravatar.cc/150?u=gray"} />
            <AvatarFallback className="bg-[#F26A1C] text-white text-2xl font-bold">
              {user?.name?.[0] || "G"}
            </AvatarFallback>
          </Avatar>
          <h2 className="text-xl font-black text-gray-900 dark:text-white">{user?.name || "Gray Johnson"}</h2>
          <p className="text-gray-500 text-sm font-medium mt-0.5">+251912345678</p>
        </div>
        ...
        <div className="flex flex-col items-center mt-4 mb-8 relative">
          <div className="relative">
            <Avatar className="w-24 h-24 border-[3px] border-white dark:border-gray-900 shadow-md">
              <AvatarImage src={user?.avatar || "https://i.pravatar.cc/150?u=gray"} />
              <AvatarFallback className="bg-[#F26A1C] text-white text-2xl font-bold">G</AvatarFallback>
            </Avatar>
          ...
          <h2 className="text-xl font-black text-gray-900 dark:text-white mt-3">{user?.name || "Gray Johnson"}</h2>
          <p className="text-[#F26A1C] text-xs font-bold mt-1 cursor-pointer hover:underline">Change Profile Picture</p>
        </div>

        <div className="flex-1 space-y-5">
          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-bold text-gray-500 uppercase px-2">Name</label>
            <SoftInput defaultValue={user?.name || "Gray Johnson"} />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-bold text-gray-500 uppercase px-2">Phone Number</label>
            <SoftInput defaultValue="+251912345678" type="tel" />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-bold text-gray-500 uppercase px-2">Email</label>
            <SoftInput defaultValue="Gray.Johnson@astu.edu.et" type="email" />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-bold text-gray-500 uppercase px-2">Default Address</label>
            <SoftInput defaultValue="B-12/A40, G-23/A14" />
          </div>
        </div>

        <div className="mt-8">
          <ActionButton onClick={() => setActiveView("menu")}>Save Changes</ActionButton>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#FDFDFD] dark:bg-gray-950 px-5 pb-28 font-sans flex flex-col">
        ...
        <div className="flex items-center gap-3 py-3 px-2 mb-4">
          ...
          <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">West Pine Avenue, Addis Ababa</p>
        </div>
        ...
        <ActionButton onClick={() => setActiveView("menu")}>Save Location</ActionButton>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#FDFDFD] dark:bg-gray-950 px-5 pb-28 font-sans flex flex-col">
        ...
        <div className="flex-1 mt-6 space-y-6">
          <div>
            <label className="text-[12px] font-bold text-gray-700 dark:text-gray-300 mb-2 block px-2">Enter Old Password</label>
            <SoftInput icon={Lock} type="password" placeholder="••••••••" />
          </div>

          <div className="pt-4 border-t border-gray-100 dark:border-gray-800">
            <label className="text-[12px] font-bold text-gray-700 dark:text-gray-300 mb-2 block px-2">Create New Password</label>
            <SoftInput icon={Lock} type="password" placeholder="Create New Password" />
            <SoftInput icon={Lock} type="password" placeholder="Confirm New Password" />
          </div>
        </div>

        <div className="mt-8">
          <ActionButton onClick={() => setActiveView("menu")}>Change Password</ActionButton>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#FDFDFD] dark:bg-gray-950 px-5 pb-28 font-sans flex flex-col">
        ...
        <div className="flex-1 mt-6 space-y-4">
          <label className="flex items-center justify-between p-4 bg-white dark:bg-gray-900 rounded-[20px] border border-orange-200 dark:border-orange-900/50 shadow-sm cursor-pointer active:scale-[0.98] transition-transform">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-sm text-gray-900 dark:text-white">CBE Birr</span>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-gray-500">+251912345678</span>
          ...
          <label className="flex items-center justify-between p-4 bg-white dark:bg-gray-900 rounded-[20px] border border-gray-100 dark:border-gray-800 cursor-pointer active:scale-[0.98] transition-transform">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-sm text-gray-900 dark:text-white">Amole</span>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-gray-500">098923456789</span>
              <ChevronRight size={16} className="text-gray-400" />
            </div>
          </label>
          ...
          >
            <span className="font-bold text-sm text-gray-700 dark:text-gray-300">Add Payment Method</span>
            <ChevronRight size={18} className="text-gray-400" />
          </button>
        </div>
      </div>
  ...
  if (activeView === "add-payment-list") {
    return (
      <div className="min-h-screen bg-[#FDFDFD] dark:bg-gray-950 px-5 pb-28 font-sans flex flex-col">
        <Header title="Add Payment Method" showBack onBackClick={() => setActiveView("payment")} />
        
        <p className="text-xs font-semibold text-gray-500 mt-2 mb-6 px-1">Choose Payment Method To Add</p>
        ...
        <div className="flex-1 space-y-4">
          ...
          <div className="flex items-center justify-between p-3 rounded-2xl bg-white dark:bg-gray-900 cursor-default">
            <div className="flex items-center gap-3">
              <img src="https://telebirr.et/wp-content/uploads/2021/05/telebirr-logo.png" alt="Telebirr" className="h-8 object-contain" />
            </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-2xl bg-white dark:bg-gray-900 cursor-default">
            <div className="flex items-center gap-3">
              <img src="https://combanketh.et/cbe_logo.png" alt="CBE" className="h-8 object-contain bg-orange-50/50 p-1 rounded-md" />
            </div>
            <div className="w-5 h-5 bg-gray-400 rounded-full flex items-center justify-center">
              <Check size={12} className="text-white" strokeWidth={3} />
            </div>
          ...
          >
            <div className="flex items-center gap-3">
              <img src="https://awashbank.com/wp-content/uploads/2020/11/awash-logo.png" alt="Awash Birr" className="h-8 object-contain" />
            </div>
  ...
  if (activeView === "add-telebirr-form") {
    return (
      <div className="min-h-screen bg-[#FDFDFD] dark:bg-gray-950 px-5 pb-28 font-sans flex flex-col">
        <Header title="Add Telebirr" showBack onBackClick={() => setActiveView("add-payment-list")} />
        
        <div className="flex-1 mt-2">
          {/* Logo & Warning */}
          <div className="flex flex-col items-center mb-8 px-4 text-center">
            <img src="https://telebirr.et/wp-content/uploads/2021/05/telebirr-logo.png" alt="Telebirr" className="h-16 object-contain mb-6" />
            <p className="text-[11px] text-gray-500 leading-relaxed font-medium">
              <b className="text-gray-900 dark:text-white">Warning:</b> Please Make Sure The Number You Enter Is Really Yours. Apart From Verifying The Number Via Otp, We Will Not Responsible For Any Inconvenience Regarding Your Account.
            </p>
          </div>
          ...
          <div className="space-y-5">
            <div>
              <label className="text-[12px] font-bold text-gray-900 dark:text-white mb-2 block">Account Name</label>
              <BorderedInput placeholder="Enter Your Name" />
            </div>

            <div>
              <label className="text-[12px] font-bold text-gray-900 dark:text-white mb-2 block">Phone Number</label>
              <BorderedInput placeholder="Enter Your Phone Number" error={true} />
              <p className="text-[10px] text-red-500 font-semibold text-center mt-1.5">Please Enter Valid Phone Number</p>
            </div>

            <div>
              <label className="text-[12px] font-bold text-gray-900 dark:text-white mb-2 block">Enter OTP For Verification</label>
              <BorderedInput placeholder="Enter OTP" error={true} />
              <div className="flex flex-col items-center mt-1.5 gap-0.5">
                <p className="text-[10px] text-red-500 font-semibold">Incorrect OTP Entered!</p>
                <p className="text-[10px] text-gray-500 font-medium">
                  Haven't Recieved OTP Yet? <span className="text-[#F26A1C] font-bold cursor-pointer">Resend</span>
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="mt-8">
          <ActionButton onClick={() => setActiveView("add-telebirr-success")}>Add Payment Method</ActionButton>
        </div>
      </div>
    );
  }
  ...
  if (activeView === "add-telebirr-success") {
    return (
      <div className="min-h-screen bg-[#FDFDFD] dark:bg-gray-950 px-5 pb-28 font-sans flex flex-col">
        <Header title="Add Telebirr" showBack onBackClick={() => setActiveView("payment")} />
        ...
        <div className="flex-1 flex flex-col items-center justify-center -mt-10 px-4 text-center">
          ...
          <h2 className="text-[15px] font-bold text-gray-800 dark:text-white leading-relaxed max-w-[280px]">
            We Have Successfully Recieved Your Submission And Processing It. We Will Let You Know Very Soon
          </h2>
        </div>

        <div className="mt-8">
          <ActionButton onClick={() => setActiveView("payment")}>Return</ActionButton>
        </div>
      </div>
    );
  }
