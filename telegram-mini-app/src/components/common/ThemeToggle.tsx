...
export function ThemeToggle({ className = "", variant = "icon" }: ThemeToggleProps) {
  ...
  if (variant === "switch") {
    return (
      <button
        onClick={toggleMode}
        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${isDark ? "bg-primary" : "bg-gray-200"
          } ${className}`}
      >
        <span
