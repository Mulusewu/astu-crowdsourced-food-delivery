...
- Create/upgrade `src/data` helpers to read flat arrays and resolve relations by IDs:
  - `getUserById`, `getCustomerProfileByUserId`, `getDelivererProfileByUserId`, `getVendorProfileByUserId`
  - `getRestaurantById`, `getCategoriesByRestaurantId`, `getMenuItemsByRestaurantId`
  - `getOrdersByCustomer`, `getOrdersByDeliverer`, `getOrdersByRestaurant`
  - `getOrderItemsByOrderId`, `getPaymentByOrderId`, `getStatusHistoryByOrderId`, `getDispatchLogsByOrderId`
